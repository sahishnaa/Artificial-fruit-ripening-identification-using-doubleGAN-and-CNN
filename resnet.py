...
path = '/content/drive/MyDrive/96 npy with gan/'

# Load the .npy files
class1_images = np.load(path + 'artificialbanana_withgan.npy')
class2_images = np.load(path + 'artificialmango_withgan.npy')
class3_images = np.load(path + 'naturalbanana_withgan.npy')
class4_images = np.load(path + 'naturalmangoe_withgan.npy')
...
# Load the dataset
# Assume the following .npy files:
# - 'images.npy': Array of image data
# - 'labels.npy': Array of corresponding labels
...
# Preprocess the data
# Resize images to 224x224 (common input size for ResNet)
...
X_train, X_val, y_train, y_val = train_test_split(images_resized, labels_categorical, test_size=0.2, random_state=42)
# Check the shapes of the split datasets
print(f'Training set shape: {X_train.shape}, {y_train.shape}')
print(f'Validation set shape: {X_val.shape}, {y_val.shape}')

# Build the ResNet model using transfer learning
